...
(load "hw-1-solution-professor.lsp")
...
(format t "Testing EXPAND...~%")
    ...
    (test-case (EXPAND 'APPLES)
               'APPLES "EXPAND_ex_A")

    (test-case (EXPAND 'CON-SENT1)
    ...
    (test-case (EXPAND 'OBJ002)
               '(HUMAN F-NAME (CHARLES)
               L-NAME ( ) GENDER (MALE)) "EXPAND_ex_2")

    (test-case (EXPAND 'UNBOUND-GAP)
               'UNBOUND-GAP "EXPAND_ex_3")

    (test-case (EXPAND 'EXPV001)
               '(SEE AGENT (HUMAN F-NAME (CHARLOTTE)
               L-NAME ( )
               GENDER (FEMALE)) OBJECT (HUMAN F-NAME (CHARLES)
               L-NAME ( ) GENDER (MALE))) "EXPAND_ex_4")
...
(format t "Testing AMEND-SF...~%")
    ...
    (test-case (AMEND-SF 'L-NAME '(NEWTON) '(HUMAN F-NAME (CHARLOTTE)))
               '(HUMAN F-NAME (CHARLOTTE)
               L-NAME (NEWTON)) "AMEND-SF_ex_1")

    (test-case (AMEND-SF 'F-NAME '(CHARLES) '(HUMAN F-NAME (CHARLOTTE)))
               '(HUMAN F-NAME (CHARLES)) "AMEND-SF_ex_2")

    (test-case (AMEND-SF 'L-NAME '(OAKLEY) AG001)
               '(HUMAN F-NAME (CHARLES)
               GENDER (MALE) L-NAME (OAKLEY)) "AMEND-SF_ex_3")
...
(format t "Testing EQUAL-SF...~%")
    ...
    (test-case (EQUAL-SF '(HUMAN)
              '(HUMAN))
               T "EQUAL-SF_ex_1")

    (test-case (EQUAL-SF '(HUMAN F-NAME (CHARLOTTE))
    '(HUMAN F-NAME (CHARLES)))
               NIL "EQUAL-SF_ex_2")

    (test-case (EQUAL-SF '(HUMAN L-NAME (NEWTON)
    F-NAME (CHARLOTTE)) '(HUMAN F-NAME (CHARLOTTE)
    L-NAME (NEWTON)))
               T "EQUAL-SF_ex_2")

    ;(test-case (EQUAL-SF '(HUMAN F-NAME 'UNBOUND-GAP1)
    ;'(HUMAN F-NAME 'UNBOUND-GAP2))
    ;           NIL "EQUAL-SF_ex_4")
